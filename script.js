...
const generateStarfield = () => {
    ...
    for(let i=0; i<150; i++) {
        ...
        const y = Math.floor(Math.random() * window.innerHeight * 2); // Taller than viewport for scrolling
        const size = Math.random() > 0.8 ? 2 : 1;
        const opacity = Math.random() * 0.5 + 0.1;
        shadows += `${x}px ${y}px 0 ${size}px rgba(255,255,255,${opacity})${i<149 ? ',' : ''}`;
    }
    const starLayer = document.createElement('div');
    starLayer.style.width = '1px';
    starLayer.style.height = '1px';
    starLayer.style.background = 'transparent';
    starLayer.style.boxShadow = shadows;
    DOM.starfield.appendChild(starLayer);
};
    ...
    planets.forEach(p => {
        p.addEventListener('click', (e) => {
            ...
            if(data) {
                ...
                // Add Saturn Ring dynamically to detail view if needed
                visual.innerHTML = '';
                if(p.getAttribute('data-planet') === 'saturn') {
                    ...
                    ring.style.left = '-30px'; ring.style.top = '35px';
...
const renderLoop = () => {
    ...
    // Parallax Starfield based on mouse movement
